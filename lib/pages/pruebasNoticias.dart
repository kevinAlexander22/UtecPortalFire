...
class _MyCarouselPageState extends State<MyCarouselPage> {
  ...
  Future<List<String>> fetchData() async {
    final response = await http.get(Uri.parse('https://www.utec.edu.sv/media/noticias/thumbs/1694027297__MG_9414.jpg'));

   if (response.statusCode == 200) {
  final List<dynamic> data = json.decode(response.body);
  final List<String> imageUrls = data.map((item) => item['image_url'].toString()).toList();
  return imageUrls;
} else {
      throw Exception('Error al cargar datos desde la página web');
    }
  }
